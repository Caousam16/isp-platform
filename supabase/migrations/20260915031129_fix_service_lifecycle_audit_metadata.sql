create or replace function public.manage_subscriber_service(p_service_id uuid,p_action text,p_plan_id uuid default null)
returns public.subscriber_services language plpgsql security definer set search_path = ''
as $function$
declare v_service public.subscriber_services; v_plan public.plans; v_previous_status text; v_previous_plan_id uuid; v_next_status text;
begin
  ...
  insert into public.audit_events(organization_id,actor_id,action,entity_type,entity_id,metadata) values(v_service.organization_id,auth.uid(),'subscriber_service.'||p_action,'subscriber_service',v_service.id,jsonb_build_object('previous_status',v_previous_status,'status',v_service.status,'previous_plan_id',v_previous_plan_id,'plan_id',v_service.plan_id));
  return v_service;
end;
$function$;;
