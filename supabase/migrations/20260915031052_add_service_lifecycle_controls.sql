...
begin
  ...
  case p_action
    ...
    when 'terminate' then
      if v_previous_status = 'terminated' then
    ...
    when 'change_plan' then
      ...
      if v_plan.id = v_service.plan_id then
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'previous_plan_id', case when p_action = 'change_plan' then to_jsonb((select plan_id from public.subscriber_services where id = v_service.id)) else to_jsonb(v_service.plan_id) end,
      'plan_id', v_service.plan_id
    )
  );
...
revoke all on function public.manage_subscriber_service(uuid, text, uuid) from public;
