...
export function Workspace({
  ...
  const subscriberById=new Map(visible.subscribers.map(s=>[s.id,s]));
  const planById=new Map(visible.plans.map(p=>[p.id,p]));
  const serviceBySubscriber=new Map(visible.services.filter(s=>s.status!=="terminated").map(s=>[s.subscriber_id,s]));
  const organizationById=new Map(data.organizations.map(o=>[o.id,o]));
  const planFor=(id:string)=>planById.get(serviceBySubscriber.get(id)?.plan_id??"");
  const subscriberFor=(id:string)=>subscriberById.get(id);
  const organizationFor=(id:string)=>organizationById.get(id)??activeOrganization;
  const unassignedSubscribers=visible.subscribers.filter(s=>!serviceBySubscriber.has(s.id));
  ...
  const billingRequestId = useRef("");
  function openModal(next: Modal) {
    billingRequestId.current=crypto.randomUUID();
    setError("");
    setModal(next);
  }
  useEffect(() => {
  ...
  async function savePassword(e: FormEvent<HTMLFormElement>) {
    ...
    setBusy(true);
    setError("");
    const passwordForm=e.currentTarget;
  ...
  async function savePayment(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setData((current) => ({
        ...current, payments: [payment, ...current.payments.filter(p=>p.id!==payment.id)], allocations: [allocation, ...current.allocations.filter(a=>a.id!==allocation.id)],
        invoices: current.invoices.map((item) => item.id === allocation.invoice_id ? { ...item, paid_minor: item.paid_minor + (current.allocations.some(a=>a.id===allocation.id)?0:allocation.amount_minor) } : item),
      }));
      setModal(null);
  ...
  useEffect(()=>setListPage(1),[view,query,filter,planFilter,speedFilter,paymentMethodFilter,auditEntityFilter,auditFrom,auditTo,billingTab,billingPeriod,billingMonth,billingFrom,billingTo,activeOrganization?.id]);
  const listCount=view==="subscribers"?matchingSubscribers.length:view==="services"?matchingServices.length:view==="plans"?matchingPlans.length:view==="audit"?matchingAudit.length:view==="billing"?(billingTab==="invoices"?matchingInvoices.length:billingTab==="payments"?matchingPayments.length:0):0;
  const pageCount=Math.max(1,Math.ceil(listCount/100));
  const currentPage=Math.min(listPage,pageCount);
  const pageRows=<T,>(rows:T[])=>rows.slice((currentPage-1)*100,currentPage*100);
  const title = portal
    ? view === "overview"
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        {!isDemo && !portal && role === "admin" && (
          <a href="/admin/routers" className="nav-item">Router snapshots</a>
        )}
      ...
      <div className="workspace">
        ...
        <main id="main-content">
          <div className="demo-notice">
            ...
            {isDemo ? "DEMO WORKSPACE" : "OPERATIONS WORKSPACE"}
            <span>
              {isDemo
                ? "Synthetic data · Changes last until you refresh · PHP sample currency"
                : "Live Supabase data · Company-scoped records"}
            </span>
            {isDemo && (
              <a href="/login">
            ...
            <section className="panel">
              ...
              <div className="table-wrap">
                <table>
                  ...
                  <tbody>
                    {pageRows(matchingSubscribers).map((s) => (
                      <tr key={s.id}>
                        <td>
                          <button
          ...
          {view === "plans" && (
            ...
            <section className="plan-grid">
              {pageRows(matchingPlans).map((p, index) => (
                <article
            ...
            <section className="panel">
              ...
              <div className="table-wrap">
                <table>
                  ...
                  <tbody>
                    {pageRows(matchingServices).map((s) => {
                          ...
                          {!portal && (
                            <td>
                              <div className="service-actions">
                                {serviceLifecycleActions(s.status, role).map(
                                  (action) => {
          ...
          {view === "billing" && (
            <section className="panel">
              ...
              ) : billingTab === "invoices" ? invoiceTable(pageRows(matchingInvoices)) : (
                <div className="table-wrap">
                  <table>
                    ...
                    <tbody>{pageRows(matchingPayments).map((payment) => {
                      const allocation = visible.allocations.find((item) => item.payment_id === payment.id);
                      const invoice = visible.invoices.find((item) => item.id === allocation?.invoice_id);
        ...
        {pageCount>1 && <nav aria-label="Table pages" className="table-pagination"><button disabled={currentPage===1} onClick={()=>setListPage(currentPage-1)}>Previous</button><span>Page {currentPage} of {pageCount} · {listCount} records</span><button disabled={currentPage===pageCount} onClick={()=>setListPage(currentPage+1)}>Next</button></nav>}
      </main>
      </div>
      {toast && (
            ...
            <div>
              ...
              <dl className="summary-list">
                ...
                {(() => {
                  ...
                  const recent = identity.network_seen_at && Date.now() - Date.parse(identity.network_seen_at) <= 360_000;
                  return <>
