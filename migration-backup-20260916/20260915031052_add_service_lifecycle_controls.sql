...
begin
  ...
  if not app_private.has_access(v_service.organization_id, array['admin'::text,'staff'::text]) then
    raise exception 'You do not have permission to manage this service' using errcode='42501';
  end if;

  v_previous_status := v_service.status;
  v_previous_plan_id := v_service.plan_id;
  v_next_status := v_previous_status;

  case p_action
    when 'activate' then
      if v_previous_status <> 'pending' then raise exception 'Only pending services can be activated' using errcode='22023'; end if;
      v_next_status := 'active';
    when 'suspend' then
      if v_previous_status <> 'active' then raise exception 'Only active services can be suspended' using errcode='22023'; end if;
      v_next_status := 'suspended';
    when 'reconnect' then
      if v_previous_status <> 'suspended' then raise exception 'Only suspended services can be reconnected' using errcode='22023'; end if;
      v_next_status := 'active';
    when 'terminate' then
      if v_previous_status = 'terminated' then raise exception 'Service is already terminated' using errcode='22023'; end if;
  ...
  set status = v_next_status,
      plan_id = case when p_action='change_plan' then v_plan.id else plan_id end,
      updated_at = now()
  where id = v_service.id
  returning * into v_service;

  insert into public.audit_events(organization_id,actor_id,action,entity_type,entity_id)
  values(v_service.organization_id,auth.uid(),'subscriber_service.'||p_action,'subscriber_service',v_service.id);
  return v_service;
end;
$$;
